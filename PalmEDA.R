...
######### BL
hr_from_bl=list()
for(d in bl_directories){hr_from_bl=c(hr_from_bl,paste(d,list.files(d,pattern = "*.peda",recursive = TRUE),sep ="")) } # check 1 recursive need not be true
...
######### PD
hr_from_pd=list()
for(d in pd_directories){hr_from_pd=c(hr_from_pd,paste(d,list.files(d,pattern = "*.peda",recursive = TRUE),sep ="")) } # check 1 recursive need not be true
...
##########  RD
hr_from_rd=list()
for(d in rd_directories){hr_from_rd=c(hr_from_rd,paste(d,list.files(d,pattern = "*.peda",recursive = TRUE),sep ="")) } # check 1 recursive need not be true
...
###########  ND
hr_from_nd=list()
for(d in nd_directories){hr_from_nd=c(hr_from_nd,paste(d,list.files(d,pattern = "*.peda",recursive = TRUE),sep ="")) } # check 1 recursive need not be true
...
##########   CD 
hr_from_cd=list()
for(d in cd_directories){hr_from_cd=c(hr_from_cd,paste(d,list.files(d,pattern = "*.peda",recursive = TRUE),sep ="")) } # check 1 recursive need not be true
...
############# ED 
hr_from_ed=list()
for(d in ed_directories){hr_from_ed=c(hr_from_ed,paste(d,list.files(d,pattern = "*.peda",recursive = TRUE),sep ="")) } # check 1 recursive need not be true
...
########### MD 
hr_from_md=list()
for(d in md_directories){hr_from_md=c(hr_from_md,paste(d,list.files(d,pattern = "*.peda",recursive = TRUE),sep ="")) } # check 1 recursive need not be true
...
########### FD 
hr_from_fd=list()
for(d in fd_directories){hr_from_fd=c(hr_from_fd,paste(d,list.files(d,pattern = "*.peda",recursive = TRUE),sep ="")) } # check 1 recursive need not be true
...
prune=function(x){ return(sum(x$Palm.EDA < 10 | x$Palm.EDA > 4700)>0)}  # 10 4700
pdf("PalmEDA.pdf",width = 15,height = 15)
par(mfrow=c(8,2)) #mar=c(2,2,2,2))
myYlabel="EDA[kOhm]"
myYLimits=c(10,4700)
yRawLimits=c(-(myYLimits[1]/500),myYLimits[2]*500)
rawTitle="Palm EDA[kOhm] raw signal sets"
cleanedTitle="Palm EDA[kOhm] valid signal sets"
#for BL
plot(all_bl_hr_frames[[1]]$Time,all_bl_hr_frames[[1]]$Palm.EDA,"l",main=rawTitle,xlab="Time[s]",ylab=myYlabel,ylim = yRawLimits)
iter=c(2:length(all_bl_hr_frames))
for(i in iter){lines(all_bl_hr_frames[[i]]$Time,all_bl_hr_frames[[i]]$Palm.EDA,"l")}
...
plot(all_bl_hr_frames[[1]]$Time,all_bl_hr_frames[[1]]$Palm.EDA,"l",main=cleanedTitle,xlab="Time[s]",ylab=myYlabel,ylim=myYLimits)
iter=c(2:length(all_bl_hr_frames))
for(i in iter){lines(all_bl_hr_frames[[i]]$Time,all_bl_hr_frames[[i]]$Palm.EDA,"l")}
legend(x="topright",legend=c("BL",s),bty="n")
#for BL 
# for PD
plot(all_pd_hr_frames[[1]]$Time,all_pd_hr_frames[[1]]$Palm.EDA,"l",main=rawTitle,xlab="Time[s]",ylab=myYlabel,ylim=yRawLimits)
iter=c(2:length(all_pd_hr_frames))
for(i in iter){lines(all_pd_hr_frames[[i]]$Time,all_pd_hr_frames[[i]]$Palm.EDA,"l")}
...
plot(all_pd_hr_frames[[1]]$Time,all_pd_hr_frames[[1]]$Palm.EDA,"l",main=cleanedTitle,xlab="Time[s]",ylab=myYlabel,ylim=myYLimits)
iter=c(2:length(all_pd_hr_frames))
for(i in iter){lines(all_pd_hr_frames[[i]]$Time,all_pd_hr_frames[[i]]$Palm.EDA,"l")}
legend(x="topright",legend=c("PD",s),bty="n")
# for PD
#for RD
plot(all_rd_hr_frames[[1]]$Time,all_rd_hr_frames[[1]]$Palm.EDA,"l",main=rawTitle,xlab="Time[s]",ylab=myYlabel,ylim=yRawLimits)
iter=c(2:length(all_rd_hr_frames))
for(i in iter){lines(all_rd_hr_frames[[i]]$Time,all_rd_hr_frames[[i]]$Palm.EDA,"l")}
...
plot(all_rd_hr_frames[[1]]$Time,all_rd_hr_frames[[1]]$Palm.EDA,"l",main=cleanedTitle,xlab="Time[s]",ylab=myYlabel,ylim=myYLimits)
iter=c(2:length(all_rd_hr_frames))
for(i in iter){lines(all_rd_hr_frames[[i]]$Time,all_rd_hr_frames[[i]]$Palm.EDA,"l")}
legend(x="topright",legend=c("RD",s),bty="n")
#for RD
#for ND
plot(all_nd_hr_frames[[1]]$Time,all_nd_hr_frames[[1]]$Palm.EDA,"l",main=rawTitle,xlab="Time[s]",ylab=myYlabel,ylim=yRawLimits)
iter=c(2:length(all_nd_hr_frames))
for(i in iter){lines(all_nd_hr_frames[[i]]$Time,all_nd_hr_frames[[i]]$Palm.EDA,"l")}
...
plot(all_nd_hr_frames[[1]]$Time,all_nd_hr_frames[[1]]$Palm.EDA,"l",main=cleanedTitle,xlab="Time[s]",ylab=myYlabel,ylim=myYLimits)
iter=c(2:length(all_nd_hr_frames))
for(i in iter){lines(all_nd_hr_frames[[i]]$Time,all_nd_hr_frames[[i]]$Palm.EDA,"l")}
legend(x="topright",legend=c("ND",s),bty="n")
#for ND
#for CD
plot(all_cd_hr_frames[[1]]$Time,all_cd_hr_frames[[1]]$Palm.EDA,"l",main=rawTitle,xlab="Time[s]",ylab=myYlabel,ylim=yRawLimits)
iter=c(2:length(all_cd_hr_frames))
for(i in iter){lines(all_cd_hr_frames[[i]]$Time,all_cd_hr_frames[[i]]$Palm.EDA,"l")}
...
plot(all_cd_hr_frames[[1]]$Time,all_cd_hr_frames[[1]]$Palm.EDA,"l",main=cleanedTitle,xlab="Time[s]",ylab=myYlabel,ylim=myYLimits)
iter=c(2:length(all_cd_hr_frames))
for(i in iter){lines(all_cd_hr_frames[[i]]$Time,all_cd_hr_frames[[i]]$Palm.EDA,"l")}
legend(x="topright",legend=c("CD",s),bty="n")
#for CD 
#for ED
plot(all_ed_hr_frames[[1]]$Time,all_ed_hr_frames[[1]]$Palm.EDA,"l",main=rawTitle,xlab="Time[s]",ylab=myYlabel,ylim=yRawLimits)
iter=c(2:length(all_ed_hr_frames))
for(i in iter){lines(all_ed_hr_frames[[i]]$Time,all_ed_hr_frames[[i]]$Palm.EDA,"l")}
...
plot(all_ed_hr_frames[[1]]$Time,all_ed_hr_frames[[1]]$Palm.EDA,"l",main=cleanedTitle,xlab="Time[s]",ylab=myYlabel,ylim=myYLimits)
iter=c(2:length(all_ed_hr_frames))
for(i in iter){lines(all_ed_hr_frames[[i]]$Time,all_ed_hr_frames[[i]]$Palm.EDA,"l")}
legend(x="topright",legend=c("ED",s),bty="n")
#for ED
#for MD
plot(all_md_hr_frames[[1]]$Time,all_md_hr_frames[[1]]$Palm.EDA,"l",main=rawTitle,xlab="Time[s]",ylab=myYlabel,ylim=yRawLimits)
iter=c(2:length(all_md_hr_frames))
for(i in iter){lines(all_md_hr_frames[[i]]$Time,all_md_hr_frames[[i]]$Palm.EDA,"l")}
...
plot(all_md_hr_frames[[1]]$Time,all_md_hr_frames[[1]]$Palm.EDA,"l",main=cleanedTitle,xlab="Time[s]",ylab=myYlabel,ylim=myYLimits)
iter=c(2:length(all_md_hr_frames))
for(i in iter){lines(all_md_hr_frames[[i]]$Time,all_md_hr_frames[[i]]$Palm.EDA,"l")}
legend(x="topright",legend=c("MD",s),bty="n")
#for MD
#for FD
plot(all_fd_hr_frames[[1]]$Time,all_fd_hr_frames[[1]]$Palm.EDA,"l",main=rawTitle,xlab="Time[s]",ylab=myYlabel,ylim=yRawLimits)
iter=c(2:length(all_fd_hr_frames))
for(i in iter){lines(all_fd_hr_frames[[i]]$Time,all_fd_hr_frames[[i]]$Palm.EDA,"l")}
...
plot(all_fd_hr_frames[[1]]$Time,all_fd_hr_frames[[1]]$Palm.EDA,"l",main=cleanedTitle,xlab="Time[s]",ylab=myYlabel,ylim=myYLimits)
iter=c(2:length(all_fd_hr_frames))
for(i in iter){lines(all_fd_hr_frames[[i]]$Time,all_fd_hr_frames[[i]]$Palm.EDA,"l")}
legend(x="topright",legend=c("FD",s),bty="n")
#for FD
