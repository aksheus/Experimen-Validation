...
GetSplitVectorEdges=function(data_frame,time_name){
  c(min(data_frame[,time_name],na.rm = TRUE),max(data_frame[,time_name],na.rm = TRUE))
}
GetSplitVectorMiddle=function(stm_frame,start_time,end_time){
  c( min(stm_frame[,start_time],na.rm = TRUE),min(stm_frame[,end_time],na.rm = TRUE),
     max(stm_frame[,start_time],na.rm = TRUE),max(stm_frame[,end_time],na.rm = TRUE))
}
...
for( i in 1:length(cd_phases_list)){ cd_phase1_means[i]=mean(cd_phases_list[[i]][[phase1]]$NR.Perinasal.EDA)}
for( i in 1:length(cd_phases_list)){ cd_phase2_means[i]=mean(cd_phases_list[[i]][[phase2]]$NR.Perinasal.EDA)}
for( i in 1:length(cd_phases_list)){ cd_phase3_means[i]=mean(cd_phases_list[[i]][[phase3]]$NR.Perinasal.EDA)}
for( i in 1:length(cd_phases_list)){ cd_phase4_means[i]=mean(cd_phases_list[[i]][[phase4]]$NR.Perinasal.EDA)}
for( i in 1:length(cd_phases_list)){ cd_phase5_means[i]=mean(cd_phases_list[[i]][[phase5]]$NR.Perinasal.EDA)}
nd_phase1_means=numeric()
nd_phase2_means=numeric()
nd_phase3_means=numeric()
nd_phase4_means=numeric()
nd_phase5_means=numeric()
for( i in 1:length(nd_phases_list)){ nd_phase1_means[i]=mean(nd_phases_list[[i]][[phase1]]$NR.Perinasal.EDA)}
for( i in 1:length(nd_phases_list)){ nd_phase2_means[i]=mean(nd_phases_list[[i]][[phase2]]$NR.Perinasal.EDA)}
for( i in 1:length(nd_phases_list)){ nd_phase3_means[i]=mean(nd_phases_list[[i]][[phase3]]$NR.Perinasal.EDA)}
for( i in 1:length(nd_phases_list)){ nd_phase4_means[i]=mean(nd_phases_list[[i]][[phase4]]$NR.Perinasal.EDA)}
for( i in 1:length(nd_phases_list)){ nd_phase5_means[i]=mean(nd_phases_list[[i]][[phase5]]$NR.Perinasal.EDA)}
...
phase1_differences=phase1_differences[!is.na(phase1_differences)]
phase2_differences=phase2_differences[!is.na(phase2_differences)]
phase3_differences=phase3_differences[!is.na(phase3_differences)]
phase4_differences=phase4_differences[!is.na(phase4_differences)]
phase5_differences=phase5_differences[!is.na(phase5_differences)]
######################### need to make vectors same length ##########################
...
pdf("PPOutput.pdf",width = 15, height= 15)
par(mfrow=c(3,1))
...
box_ylabel="Perinasal Perspiration[oC^2] Phase-wise Means"
legend_text="CD"
ylim_upper=(max(apply(dfphases,2,max,na.rm=TRUE))+5)
ylim_lower=(min(apply(dfphases,2,min,na.rm=TRUE))-5)
bxp=boxplot(dfphases,main=box_title,ylab=box_ylabel,col=c(nostress_color,stress_color,nostress_color,stress_color,nostress_color))
abline(h=positive_displacement,lwd=2)
legend(x="topright",legend=legend_text,bty="n")
xcoords=c(1:5)
ycoords=apply(bxp$stats,2,max)
ntext=paste("n=",bxp$n)
text(x=xcoords,y=10.010,labels = ntext,offset = 0.9,cex=1.2)
if(length(stars)>0){
  text(x=xcoords,y=10.0107,labels = stars,offset = 0.9,cex=2.5) } 
...
for( i in 1:length(ed_phases_list)){ ed_phase1_means[i]=mean(ed_phases_list[[i]][[phase1]]$NR.Perinasal.EDA)}
for( i in 1:length(ed_phases_list)){ ed_phase2_means[i]=mean(ed_phases_list[[i]][[phase2]]$NR.Perinasal.EDA)}
for( i in 1:length(ed_phases_list)){ ed_phase3_means[i]=mean(ed_phases_list[[i]][[phase3]]$NR.Perinasal.EDA)}
for( i in 1:length(ed_phases_list)){ ed_phase4_means[i]=mean(ed_phases_list[[i]][[phase4]]$NR.Perinasal.EDA)}
for( i in 1:length(ed_phases_list)){ ed_phase5_means[i]=mean(ed_phases_list[[i]][[phase5]]$NR.Perinasal.EDA)}
nd_phase1_means=numeric()
nd_phase2_means=numeric()
nd_phase3_means=numeric()
nd_phase4_means=numeric()
...
ycoords=apply(bxp$stats,2,max)
ntext=paste("n=",bxp$n)
text(x=xcoords,y=10.008,labels = ntext,offset = 0.9,cex=1.2)
if(length(stars)>0){
  text(x=xcoords,y=10.0087,labels = stars,offset = 0.9,cex=2.5)}
############################## subsection plottong boxplot ###########################################
################# @@@@@@@@@@@@@@@@@@@@@@@@ END @@@@@@@@@@@@@@@  #################################################
...
for( i in 1:length(md_phases_list)){ md_phase1_means[i]=mean(md_phases_list[[i]][[phase1]]$NR.Perinasal.EDA)}
for( i in 1:length(md_phases_list)){ md_phase2_means[i]=mean(md_phases_list[[i]][[phase2]]$NR.Perinasal.EDA)}
for( i in 1:length(md_phases_list)){ md_phase3_means[i]=mean(md_phases_list[[i]][[phase3]]$NR.Perinasal.EDA)}
for( i in 1:length(md_phases_list)){ md_phase4_means[i]=mean(md_phases_list[[i]][[phase4]]$NR.Perinasal.EDA)}
for( i in 1:length(md_phases_list)){ md_phase5_means[i]=mean(md_phases_list[[i]][[phase5]]$NR.Perinasal.EDA)}
nd_phase1_means=numeric()
nd_phase2_means=numeric()
nd_phase3_means=numeric()
nd_phase4_means=numeric()
